...
async function updateFrameFromKnob(degrees) {   
    ...
    // Ottieni il frame dalla cache invece di caricarlo direttamente dal percorso
    if (editModeActive){
...
async function onDrag(e) {
    if (isDragging) {
        if (currentKnob !== null) {
            ...
            if (editModeActive) {
                if (currentKnob === 0) {
                    // Usa la funzione per ottenere il frame dalla cache
                    updateFrameFromKnob(angles[0]);
                } else if (currentKnob === 1) {
                    // Aggiungi la logica per recuperare il frame dalla cache per il secondo knob
